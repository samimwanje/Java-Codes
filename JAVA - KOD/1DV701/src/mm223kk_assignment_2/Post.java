...
public class Post {
	...
	/**
	 * This function is used to handle an upload from the user,
	 * it may not be perfect but it checkes the header of the upload
	 * and verifies that it it a real .png image. It also discards upload
	 * that are not .png. 
	 * @param boundary
	 * @param contentLength
	 */
	public void handlePost(int contentLength) {
		 try {	
			 	...
			 	int bufferSize = Math.min(2048,contentLength);
		        byte[] buffer = new byte[bufferSize];					// Bytes to write/read per loop. Either 2048 or the file size if less.
		        ...
		        while (true) {											// read/write buffer until the entire file has been written.
		            ...
		            if(length != bufferSize)							// Break when length is no longer buffer size.
		            	break;
		        ...
		        if(!checkType(head)) {
			    	...
			    	System.out.println("Unsportted media by:  "+ socket.getInetAddress().getHostAddress() + ":" + socket.getPort());	//Log unsupported in console.
		        	Errors.errorHandle("415 Unsupported Media Type", "Error 415: The image you are trying to upload has the wrong type");														// Redirect user to fail page.
		        }else {	
		        	redirect("302 Found", "/upload/success.html?code="+imageName); 											// Redirect user to success page.
		        	fileOutputStream.close();																				// Close the image write stream.
		        	System.out.println("User "+socket.getInetAddress().getHostAddress() + ":" + socket.getPort()+" uploaded: "+imageName);
               }	
		} catch (Exception e) {
				e.printStackTrace();
	...
	private boolean checkType(String  head){
		try {	
			...
			e.printStackTrace();
			Errors.internalError(); 	// If something wrong happens here, send an internal error to the user.
		}
	...
	public void tooLarge() {	
	 ...
	 public void redirect(String status, String redirection) {
			try {
